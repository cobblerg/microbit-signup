/**
 * ====================================================================
 * [구글 스프레드시트용 Apps Script 코드 (신청 및 취소 완벽 처리)]
 * 
...
function doPost(e) {
  ...
  lock.tryLock(10000); // 10초 대기 잠금

  try {
    const sheet = SpreadsheetApp.getActiveSpreadsheet().getActiveSheet();
    
    // 시트 첫 줄(헤더)이 비어있으면 자동으로 생성합니다.
    if (sheet.getLastRow() === 0) {
      sheet.appendRow(["신청일시", "이름", "학교", "전화번호"]);
    }

    const data = JSON.parse(e.postData.contents);
    const action = data.action || "apply";
    ...
    if (action === "cancel") {
      ...
      // 하이픈 및 특수문자 제거 후 숫자만 비교 (예: 01012345678)
      ...
      for (let i = 1; i < rows.length; i++) {
        ...
        if (rowName === targetName && rowPhone === targetPhone) {
          foundRowIndex = i + 1; // 실제 구글 시트의 행 번호 (1부터 시작)
          break;
        }
      }
      ...
      // 일치하는 행을 완전히 삭제 (정원 1자리 자동 확보)
      sheet.deleteRow(foundRowIndex);

      const remainingCount = Math.max(0, sheet.getLastRow() - 1);
      return makeResponse(
        "success", 
        data.userName + "님의 참가 신청이 성공적으로 취소되었습니다.<br>(현재 접수 인원: " + remainingCount + " / " + MAX_APPLICANTS + "명)"
      );
    }
